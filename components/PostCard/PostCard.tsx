...
const PostCard: FC<PostCardProps> = ({ title, description, slug, img }) => {
  const buttonStyle =
    "hover:bg-orange-300 bg-orange-400 transition-transform duration-200 hover:scale-105 text-white my-2 py-1 px-3 rounded-md font-sans";
    ...
    <div className="border-2 border-gray-200 rounded-md h-96 w-80 overflow-hidden">
      ...
      <div className="flex flex-col h-52 justify-between pt-2 px-2 ">
        ...
        <Link prefetch={false} href={`/posts/${slug}`}>
          <button className={buttonStyle} aria-label={`Číst článek: ${title}`}>
